            ...
            <div class="row">
                <div class="col-md-2">
                    ...
                    <c:forEach items="${requestScope.lsBrand}" var="brand">
                        <a class="mt-3" style="text-transform: uppercase; font-weight: 500; 
                           margin-left: 15px;" href="filter?id=${brand.id}&type=1">${brand.name}</a>
                    </c:forEach>
                    <h4 class="mt-5" style="color: #098;">Chất liệu</h4>
                    <c:forEach items="${requestScope.lsMaterial}" var="lsMarterial">
                        <a class="mt-3" style="text-transform: uppercase; font-weight: 500; margin-left:
                           15px;" href="filter?id=${lsMarterial.id}&type=4">${lsMarterial.material}</a>
                    </c:forEach>
                </div>
                            ...
                            <c:forEach items="${requestScope.lsProduct}" var="product">
                                <div class="col-md-3 mt-4 ">
                                    <div class="card" style="width: 252px; height: 350px;">
                                        <img src="asset/product/${product.imgName}" style="width: 250px;" 
                                             class="card-img-top">
                                        <div class="card-body">
                                            <h5 class="card-title">
                                                <a  href="product-detail?id=${product.id}" style="
                                                    text-decoration: none; color: #000;">${product.name}</a>
                                            </h5>
                                            <h5><fmt:formatNumber type="number" maxFractionDigits="3"
                                                              value="${product.price}"/><sup>đ</sup></h5>
                                            <a href="" class="btn btn-primary mt-2">Mua ngay</i></a>
                                            <a style="float: right;" href="cart?id=${product.id}" 
                                               class="btn btn-danger mt-2"><i class="fas fa-cart-plus"></i></a>
                                        </div>
                                    </div>
                                </div>
                            </c:forEach>
